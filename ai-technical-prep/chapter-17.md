...
## Edition 4.6 Addendum: Verifiable Model Supply Chains and Promotion Gates

An AI release is a graph of artifacts, not a single model file. The promotion record must bind source code, build environment, base model, datasets, training configuration, adapters, tokenizer, evaluation evidence, container image and deployment policy. If any edge is unverifiable, “the evaluated model” may not be the model serving production traffic.

### Provenance statement

Use an in-toto/SLSA-style provenance predicate generated by the build or training platform rather than handwritten by the developer. It should identify:

- immutable source and pipeline revisions;
- builder identity and isolated execution environment;
- base-model and dependency digests;
- dataset manifest or Merkle-root digest, subject to privacy constraints;
- hyperparameter and training-code digests;
- output model, tokenizer and container-image digests;
- evaluation record and policy bundle used for approval.

SLSA Build levels describe properties of the build provenance and platform; writing a JSON file does not make a pipeline “Level 3.” Verify the required platform controls and use precise claims.

### Signing and transparency

Keyless signing can exchange a workload's short-lived OIDC identity for an ephemeral signing certificate and record the signature in a transparency log. The security value comes from verifying issuer, subject, workflow identity, artifact digest and transparency evidence—not merely observing that a signature exists.

Admission policy should require the expected signer and provenance predicate for both the serving image and model artifact. Pin by digest. Tags such as `approved` or `latest` are mutable labels and must not become the authorization boundary.

### Promotion as a state machine

```text
BUILT -> QUARANTINED -> EVALUATED -> APPROVED -> CANARY -> PRODUCTION
  |          |             |            |           |
  +----------+-------------+------------+-----------+-> REVOKED
```

Only the promotion service may move an artifact between states. A release transaction records artifact digests, approvals, evaluation bounds, exceptions, deployment target and rollback pointer. Runtime detection can revoke a digest and prevent rescheduling even if an old tag still references it.

Rollback must be tested as a complete dependency set. Restoring model weights while retaining an incompatible tokenizer, prompt template, feature transform or policy bundle is not a safe rollback.

### Confidential-computing boundaries

Memory encryption and attestation can reduce exposure to a host or cloud operator on supported platforms, but they do not establish that training data was authorized, that model behavior is safe or that every accelerator and interconnect is covered. Document the attested boundary, unsupported devices, key-release policy and performance cost. Protect distributed training traffic and coordinator identities independently.

### Staff/Principal interview drill

**Prove that production serves the artifact that passed evaluation without long-lived signing keys.** Use workload identity, ephemeral signing certificates, immutable artifact digests, build-generated provenance, transparency logging and admission verification. Bind the evaluation record to the exact model/tokenizer/image tuple, require promotion-service authorization, and test revocation and atomic rollback. State what the evidence proves—and what it cannot prove about dataset legality or hidden model behavior.
